...
rtp2httpd_remove_network_profiles() {
    ...
    for uuid in "${MULTICAST_PROFILE_UUID:-}" "${FCC_PROFILE_UUID:-}"; do
        if [[ ! "$uuid" =~ ^[0-9a-fA-F-]{36}$ ]]; then
            echo "❌ 状态文件中的 profile UUID 无效，拒绝删除：${uuid:-空}"
            failed=1
            continue
        fi
        if ! nmcli connection show uuid "$uuid" >/dev/null 2>&1; then
            echo "ℹ️ VLAN profile 已不存在：$uuid"
            continue
        fi
        if ! nmcli connection delete uuid "$uuid" >/dev/null 2>&1 || nmcli connection show uuid "$uuid" >/dev/null 2>&1; then
            echo "❌ 无法删除或确认删除 VLAN profile：$uuid"
            failed=1
            continue
        fi
        echo "✅ 已删除 VLAN profile：$uuid"
    done
    return "$failed"
}
