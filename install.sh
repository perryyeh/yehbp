...
APP_VERSION="2026.09.01.01"
REPO_URL="https://github.com/perryyeh/yehbp"
RAW_GITHUB_BASE="https://raw.githubusercontent.com/perryyeh/yehbp/main"
RAW_INSTALL_URL="${RAW_GITHUB_BASE}/install.sh"
RAW_VERSION_URL="${RAW_GITHUB_BASE}/VERSION"
...
fetch_remote_yehbp_version() {
    ...
    if ! require_curl_for_configured_proxy; then
        ...
        yehbp_curl --connect-timeout 10 --max-time 10 -fsSL "$url" 2>/dev/null | tr -d '[:space:]'
    elif command -v wget >/dev/null 2>&1; then
...
install_mihomo() {
    ...
    if [ "$MIHOMO_NETWORK_MODE" = "macvlan" ] && [ ! -f config.replace.yaml ]; then
        echo "❌ 缺少 Mihomo 订阅覆盖模板：config.replace.yaml"
        return 1
    fi
    ...
    if [ "$MIHOMO_NETWORK_MODE" = "macvlan" ] && [ -f "config.yaml" ] && [ -n "$gateway" ] && [ "$gateway" != "null" ]; then
        ...
        sed -i "s/10.0.0.1/${gateway}/g" config.replace.yaml
    fi
    ...
    if [ "$MIHOMO_NETWORK_MODE" = "macvlan" ]; then
        ...
        if [ "$USE_IPV6" -eq 0 ]; then
            ...
            remove_config_ipv6_fields config.replace.yaml
        fi
    else
        rm -f "$WORK_DIR/.env"
    fi
...
sync_dockcheck_auto_update_components() {
    ...
    if [ -z "$remote_version" ]; then
        echo "⚠️ 无法识别上游 Dockcheck 版本；保留当前 Dockcheck，仅同步 Dockcheck 配套组件。"
    else
        ...
        if dockcheck_version_gt "$remote_version" "$local_version"; then
            ...
            echo "ℹ️ Dockcheck 未发现更高版本，保留当前版本：$local_version"
        fi
    fi

    echo "🔄 更新 Dockcheck 配套组件"
